...
function PreviewNavigation({ cor, dark = false }: { cor: string; dark?: boolean }) { return <div className="mt-5 space-y-3"><div className="flex items-center gap-2 text-xs"><LayoutDashboard className="h-4 w-4" /><span>Painel</span></div><div className="flex items-center gap-2 rounded-md p-2 text-xs" style={{ backgroundColor: cor, color: dark ? "var(--sidebar)" : "var(--primary-foreground)" }}><MessageCircle className="h-4 w-4" /><span>WhatsApp</span></div><div className={dark ? "h-8 rounded-md border border-sidebar-border" : "h-8 rounded-md border"} /></div>; }